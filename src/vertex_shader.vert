precision highp float;
varying vec3 vl;
varying vec2 vuv;
attribute vec 3 p;
attribute vec 2 uv;
attribute vec 3 n;
uniform vec3 cam;
uniform float l[7*max_lights];

const max_lights = 16

const mat4 v=mat4(1,0,0,0,0,.707,.707,0,0,-.707,.707,0,0,-22.627,-22.627,1); // view
const mat4 r=mat4(.977,0,0,0,0,1.303,0,0,0,0,-1,-1,0,0,-2,0); // projection

void main(void){
    vl=vec3(0.3,0.3,0.6); // ambient color
    for(int i=0; i<max_lights; i++) {
        vec3 lp=vec3(l[i*7],l[i*7+1],l[i*7+2]); // light position
        vl+=vec3(l[i*7+3],l[i*7+4],l[i*7+5]) // light color *
            *max(dot(n,normalize(lp-p)),0.) // diffuse *
            *(1./(l[i*7+6]*( // attentuation *
                length(lp-p) // distance
            ))); 
    }
    vuv=uv;
    gl_Position=r*v*(vec4(p+cam,1.));
}
